...
const tryJSONParse = (str, fallback = null) => {
  try {
...
const fts5Statements = {
  // FTS5 Autocomplete queries - Get scores first, then join for details with weighted scoring
  coursesAutocompleteFTS5: db.prepare(`
    SELECT fts.course_code, fts.course_name, fts.class_desc, fts.oscar_title, fts.class_id,
           ...
           (bm25(courses_fts) * 0.7 + LOG(COALESCE(NULLIF(c.total_students, 0), 1)) * 0.3) as weighted_score
  ...
  // FTS5 Full search queries - Get scores first, then join for details with weighted scoring
  ...
  coursesSearchFTS5Fast: db.prepare(`
    SELECT class_id, bm25(courses_fts) as relevance_score
    FROM courses_fts 
    WHERE courses_fts MATCH ?
  ...
  // Batch lookup by multiple class_ids after FTS5 match
  courseDetailsByIds: db.prepare(`
    SELECT id, dept_abbr, course_num, dept_abbr || ' ' || course_num AS class_name,
           class_desc, total_students, total_grades
    FROM classdistribution
    WHERE id = ?
  ...
  // Prepared statement for batch lookups (will be dynamically created)
  courseDetailsBatch: null
};

// Helper function for optimized batch course lookups
const getBatchCourseDetails = (courseIds) => {
  if (!courseIds || courseIds.length === 0) return [];
  
  if (courseIds.length === 1) {
    return [fts5Statements.courseDetailsByIds.get(courseIds[0].class_id)].filter(Boolean);
  }
  
  // Create batch query for multiple IDs
  ...
  const courses = batchStmt.all(...ids);
...
// Helper function to convert search query to FTS5 format with focused LIKE fallback
const toFTS5Query = (search) => {
  if (!search || !search.trim()) return null;
  
  const trimmed = search.trim();
  
  // ONLY 3 scenarios should use LIKE fallback - everything else uses FTS5
  ...
  // EVERYTHING ELSE uses FTS5 - no other fallbacks!
  
  // Escape FTS5 special characters
  const escapeFTS5Query = (str) => {
    return str.replace(/"/g, '""');
  };
  
  // Enhanced course code detection with multiple patterns
  // More flexible - handles various formats
  const courseCodeExact = trimmed.match(/^([A-Z]{2,6})\s*(\d{3,5}[A-Z]?)$/i);
  const courseCodePartial = trimmed.match(/^([A-Z]{2,6})\s*(\d{1,4})?$/i);
  
  if (courseCodeExact) {
    const dept = courseCodeExact[1].toUpperCase();
    const number = courseCodeExact[2];
    
    // CRITICAL: Handle both "CS1331" and "CS 1331" formats ALWAYS
    return {
      query: `"${escapeFTS5Query(dept + number)}" OR "${escapeFTS5Query(dept + ' ' + number)}"`,
      type: 'exact_course',
      boost: true,
      priority: 1000,
      useFastQuery: true  // Flag for optimized query path
    };
  }
  ...
  if (courseCodePartial) {
    ...
    if (partialNumber) {
      ...
      // Handle both formats for partial matches too
      return {
        query: `${escapeFTS5Query(dept + partialNumber)}* OR ${escapeFTS5Query(dept + ' ' + partialNumber)}*`,
        type: 'partial_course',
        boost: true,
        priority: 800
      };
  ...
  if (/^[A-Z]+$/i.test(trimmed)) {
    return {
      query: `${escapeFTS5Query(trimmed.toUpperCase())}*`,
      type: 'dept_prefix',
      boost: true,
      priority: 900
    };
  ...
  // Single words - always use FTS5, remove length restrictions
  // Use phrase search for terms with special characters
  if (/[!"#%&'()*+,\-.\/:;<=>?@\[\\\]^_`{|}~]/.test(trimmed)) {
    return {
      query: `"${escapeFTS5Query(trimmed)}"`,
      type: 'phrase',
  ...
  // For remaining single words (including single characters), use prefix search
  // This includes: single numbers ("1"), single symbols ("!"), short words ("CS"), long words ("programming")
  return {
    query: `${escapeFTS5Query(trimmed)}*`,
    type: 'prefix',
    boost: false,
...
export const getAutocompleteFTS5 = (search) => {
  ...
  try {
    ...
    // FTS5-First Strategy: Always try to use FTS5 unless completely impossible
    ...
    if (fts5QueryObj) {
      ...
      try {
        ...
        if (fts5QueryObj.useFastQuery && fts5QueryObj.type === 'exact_course') {
          // Fast two-step path: FTS5-only query, then batch lookup
          const courseMatches = fts5Statements.coursesAutocompleteFTS5Fast.all(fts5Query);
          courses = getBatchCourseDetails(courseMatches);
          
          // For exact matches, usually no professors or departments needed
          professors = [];
          departments = [];
        } else {
        ...
        // Only fall back on actual FTS5 syntax errors, not performance assumptions
        console.warn("FTS5 autocomplete syntax error, falling back to LIKE:", fts5Error.message);
      ...
      // Very rare case - only when toFTS5Query returns null (empty queries)
      const searchParam = `%${search.replace(/ /g, "")}%`;
      courses = statements.coursesAutocomplete.all(searchParam, searchParam);
      professors = statements.professorsAutocomplete.all(searchParam);
      departments = statements.departmentsAutocomplete.all(searchParam, searchParam);
...
export const getSearchFTS5 = (search) => {
  ...
  try {
    ...
    // FTS5-First Strategy: Always try to use FTS5 unless completely impossible
    // Based on benchmarks: FTS5 is 2-487x faster when actually used
    const fts5QueryObj = toFTS5Query(search);
    ...
    if (fts5QueryObj) {
      ...
      try {
        ...
        if (fts5QueryObj.useFastQuery && fts5QueryObj.type === 'exact_course') {
          // Fast two-step path: FTS5-only query, then batch lookup
          const courseMatches = fts5Statements.coursesSearchFTS5Fast.all(fts5Query);
          courses = getBatchCourseDetails(courseMatches);
        ...
        // Only fall back on actual FTS5 syntax errors, not performance assumptions
        console.warn("FTS5 search syntax error, falling back to LIKE:", fts5Error.message);
        const searchParam = `%${search.replace(/ /g, "")}%`;
